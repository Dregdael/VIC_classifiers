...
namespace VIC
{
    class Clustering
    {
        ...
        private int clusters2idx = 0;

        private int clusters3idx_1 = 1;
        private int clusters3idx_2 = 2;

        private int calls2 = 0;
        private int calls3 = 0;


        //########################Constructor#################################################
        //public Clustering(string data_path= @"D:\Code\Migue\Assignment_2\VIC\VIC\data.csv")
        public Clustering(string data_path= @"D:\Actividades\Maestría\Segundo semestre\Machine learning\Assignment_2\VIC-master\VIC-master\VIC\data2.csv")
        {
        ...
        public IDataView get2clustered()
        {
            if (++calls2 > 50)
            {
                System.Console.WriteLine("More than 50 calls without reset (2-clustered)");
            }
            float clusters_split = (sorted_data.Count / 51.0f) * ++clusters2idx;
